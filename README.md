...
# Instructions
- Install `uv` if you don't already have it - follow instructions from their [installation page](https://docs.astral.sh/uv/getting-started/installation/)
- Download the solver files - `git clone https://github.com/ChrisMusson/LOI_Solver.git LOI26 & cd LOI26`
- Install requirements - `uv sync`
- Replace the file `xpts.csv` with an updated xpts CSV
- Edit settings in `settings.json` - most importantly, you need to give the solver a way of finding your current squad, current amount itb, and the gameweek to solve from. You can do this in two ways:
  ...
  1. Supply your `current_squad`, `initial_itb`, and `next_gw` manually - add `"next_gw": gw,`, `"initial_itb": itb`, and `"current_squad": [A, B, ..., N, O],` to `settings.json`, where `gw` is the gameweek you want to solve from, itb is your intial amount in the bank, and `[A, B, ..., N, O]` is a 15-integer-long list of the IDs of the 15 players in your current squad. These IDs must match up with the IDs in your `xpts.csv` file

  I would personally recommend using option 1 here
...
- Run the solver - `uv run solve.py`
...
- `itb_change_per_gw` - this is the amount to increase or decrease your budget in an attempt to account for future price changes. E.g. `"itb_change_per_gw": 0.1` will increase your total budget by 0.1 every gameweek, and `"itb_change_per_gw": -0.05` will make you lose 0.1 every two gameweeks.

- `position_constraints` - this allows you to enforce custom rules on how many players from specific teams and positions must (or must not) appear in your team.

  | Field | Type | Description | Default |
  |-------|------|-------------|---------|
  | `teams` | list[str] | Team names (must match exactly the `team` column in `xpts.csv`) | required |
  | `pos` | string | Position letters from `GDMF` (e.g. `"D"`, `"MF"`, `"GDMF"`) | `"GD"` |
  | `gws` | list[int] | Gameweeks the rule applies to | `[next_gw]` |
  | `num` | int | Number of players | `1` |
  | `type` | `"min" \| "max" \| "exact"` | Minimum / maximum / exact count | `"min"` |
  | `squad_lineup` | `"squad" \| "lineup"` | Apply to full squad or starting XI | `"squad"` |


  ## Examples

  ### 1) At least 2 Galway GKs/DEFs in GW7 in the starting XI
  ...
  ### 2) At most 1 player from Drogheda or Dundalk in the entire squad for GWs 7-11
...
Everything else is mostly the same as the FPL solver. To clarify the differences:
...
- do `uv run simulations.py --no 50 --parallel 6` where 50 is the total number of simulations you want to run, and 6 is the number of solves to run in parallel. If not supplying a value for --parallel, it will use a value based on the number of CPU processes available
- when this has finished, do `uv run sensitivity.py` and answer the questions, or do e.g. `uv run sensitivity.py --all_gws n --gw 5 --wildcard n`. You can do `uv run sensitivity.py --help` for help with the optional command line arguments.