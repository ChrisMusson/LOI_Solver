    ...
    if all_gws == "y":
        ...
        for filename in Path(directory).glob("*.csv"):
            ...
            for week in gameweeks:
                if plan[(plan["week"] == week) & (plan["t_in"] == 1)]["name"].to_list() == []:
                    ...
                    buy_list = plan[(plan["week"] == week) & (plan["t_in"] == 1)]["name"].to_list()
                    for buy in buy_list:
                        buys.append({"move": buy, "iter": iter, "week": week})

                    sell_list = plan[(plan["week"] == week) & (plan["t_out"] == 1)]["name"].to_list()
        ...
        if situation == "n":
            ...
            for filename in Path(directory).glob("*.csv"):
                ...
                try:
                    ...
                    iter = 0
                if plan[(plan["week"] == gw) & (plan["t_in"] == 1)]["name"].to_list() == []:
                    buys.append({"move": "No transfer", "iter": iter})
                    sells.append({"move": "No transfer", "iter": iter})
                    move.append({"move": "No transfer", "iter": iter})
                else:
                    buy_list = plan[(plan["week"] == gw) & (plan["t_in"] == 1)]["name"].to_list()
                    buy = ", ".join(buy_list)
                    buys.append({"move": buy, "iter": iter})
                    sell_list = plan[(plan["week"] == gw) & (plan["t_out"] == 1)]["name"].to_list()
                    sell = ", ".join(sell_list)
            ...
            for filename in Path(directory).glob("*.csv"):
                ...
                goalkeepers += (
                    plan[(plan["week"] == gw) & (plan["pos"] == "G") & (plan["t_out"] != 1)][["name", "lineup"]]
                    .apply(lambda x: (x["name"], 1 if x["lineup"] == 1 else 0), axis=1)
                    .to_list()
                )
                # Defenders list of tuples (name, lineup status)
                defenders += (
                    plan[(plan["week"] == gw) & (plan["pos"] == "D") & (plan["t_out"] != 1)][["name", "lineup"]]
                    .apply(lambda x: (x["name"], 1 if x["lineup"] == 1 else 0), axis=1)
                    .to_list()
                )
                # Midfielders list of tuples (name, lineup status)
                midfielders += (
                    plan[(plan["week"] == gw) & (plan["pos"] == "M") & (plan["t_out"] != 1)][["name", "lineup"]]
                    .apply(lambda x: (x["name"], 1 if x["lineup"] == 1 else 0), axis=1)
                    .to_list()
                )
                # Forwards list of tuples (name, lineup status)
                forwards += (
                    plan[(plan["week"] == gw) & (plan["pos"] == "F") & (plan["t_out"] != 1)][["name", "lineup"]]
                    .apply(lambda x: (x["name"], 1 if x["lineup"] == 1 else 0), axis=1)
                    .to_list()
                )
                no_plans += 1
...
if __name__ == "__main__":
    ...
    try:
        ...
        parser.add_argument(
            "--wildcard",
            choices=["Y", "y", "N", "n"],
            help="'y' if using wildcard or a preseason solve, 'n' otherwise",
        )
